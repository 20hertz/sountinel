...
export const AUDIO_FORMATS = ['mp3', 'wav', 'ogg', 'flac', 'm4a', 'aac', 'opus'];
 ...
 */
export async function validateGoogleDriveLink(driveId, apiKey, requestOrigin) {
    try {
        ...
        const audioCheck = await checkAudioContent(driveId, fileType, apiKey, requestOrigin, metadataResult.metadata);
 ...
 */
async function fetchDriveMetadata(driveId, apiKey, requestOrigin) {
    try {
 ...
 */
function detectFileType(metadata) {
 ...
 */
async function checkAudioContent(driveId, fileType, apiKey, requestOrigin, metadata) {
    if (fileType === 'folder') {
 ...
 * @returns AudioCheckResult with audio presence and count
 */
async function checkFolderAudioContent(folderId, apiKey, requestOrigin) {
    try {
        ...
        const audioCount = files.filter(f => AUDIO_FORMATS.some(ext => f.name.toLowerCase().endsWith(`.${ext}`))).length;
        return {
 ...
 * @returns AudioCheckResult with audio presence and count
 */
function checkSingleFileAudio(metadata) {
    if (!metadata) {
        return { hasAudio: false, audioCount: 0 };
    }
